# borrowed from glmnet, minor changes
cv.lognet <- function(predmat, y, type.measure, weights, foldid, grouped) {
    ...
    if ((N/nfolds < 10) && type.measure == "auc") {
        stop("Too few (< 10) observations per fold for type.measure='auc' in
             cv.lognet; use type.measure='deviance' instead. Alternatively,
             use smaller value for nfolds")
    }
